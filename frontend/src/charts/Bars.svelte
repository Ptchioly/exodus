...
<div
  ...
  style="height: {isDetailed
    ? bars.bars.length * 4 * (bars.conf.detailedSpace / 100)
    : 4}em"
>
  ...
  <section class="bars">
    {#each bars.bars as bar, i}
      <div
        ...
        style="top: {isDetailed
          ? i * bars.conf.detailedSpace
          : 0}%; z-index: {i * 1};"
        data-index={i + 1}
        ...
        <div class="bar" style="width: {percentOf(bar.value)}%">
          ...
          >
            ...
            <span class="label--detailed"
              >{bar.detailedLabel
                ? bar.detailedLabel.replace('${value}', `${bar.value}`)
                : ''}</span
            >
          </div>
        </div>
